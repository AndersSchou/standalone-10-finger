...
export class CourseHelperService {
  ...
  private closeDetailsModalObservable =
    this.closeDetailsModalSource.asObservable();
      ...
      if (cat && cat.categories) {
        ...
        if (localStorage.getItem(STORAGE_KEY_TYPE.COURSES_PROGRESS)) {
          const storedData = JSON.parse(
            localStorage.getItem(STORAGE_KEY_TYPE.COURSES_PROGRESS) as string
          );
          const findData = storedData.find(
            (el: StoredCoursesDTO) => el.language === lang
          );
          if (findData) {
   ...
   * @returns A CoursesDTO object.
   */
  mapCourseDataWithStoredData(
    course: CoursesDataDTO,
    storedData: StoredCoursesDTO
  ): CoursesDTO {
    const categories: ExtendedCategoryDTO[] = course.categories.map(
      (category: CategoryDataDTO) => {
        const storedCategory = storedData.data[0].categories.find(
          (el: StoredCategoryDataDTO) => el.id === category.id
        );
        if (storedCategory) {
          const courses = category.courses.map((course: CourseDataDTO) => {
            const storedCourse = storedCategory.courses.find(
              (el: StoredCourseDataDTO) => el.id === course.id
            );
            if (storedCourse) {
              const exercises = course.exercises.map(
                (exercise: CourseExerciseDataDTO) => {
                  const storedExercise = storedCourse.exercises.find(
                    (el: StoredCourseExerciseDataDTO) => el.id === exercise.id
                  );
                  if (storedExercise) {
                    return {
                      ...exercise,
                      ...
                      results: storedExercise.results
                        ? storedExercise.results
                        : [],
   ...
   * @returns A CoursesDTO object.
   */
  convertCoursesDataDTOToCoursesDTO(
    courses: CoursesDataDTO,
    language: string
  ): CoursesDTO {
    const cats: ExtendedCategoryDTO[] = courses.categories.map(
      (category: CategoryDataDTO) => {
        const courses = category.courses.map((course: CourseDataDTO) => {
          const exercises = course.exercises.map(
            (exercise: CourseExerciseDataDTO) => {
  ...
  updateLocalStorageData(courses: CoursesDTO): void {
    ...
    if (localStorage.getItem(STORAGE_KEY_TYPE.COURSES_PROGRESS)) {
      ...
      const findData = storedData.find(
        (el: StoredCoursesDTO) => el.language === courses.language
      );
      if (findData) {
      ...
      localStorage.setItem(
        STORAGE_KEY_TYPE.COURSES_PROGRESS,
  ...
  getLatestCategory(categories: ExtendedCategoryDTO[]): ExtendedCategoryDTO {
    return categories.reduce(
      (prev: ExtendedCategoryDTO, current: ExtendedCategoryDTO) => {
  ...
  getLatestCourse(category: ExtendedCategoryDTO): ExtendedCourseDTO {
    return category.courses.reduce(
      (prev: ExtendedCourseDTO, current: ExtendedCourseDTO) => {
        if (current.updatedAt) {
          if (!prev || !prev.updatedAt) {
            return current;
          }
  ...
  getLatestExercise(course: ExtendedCourseDTO): ExtendedCourseExerciseDTO {
    return course.exercises.reduce(
      (prev: ExtendedCourseExerciseDTO, current: ExtendedCourseExerciseDTO) => {
        if (current.updatedAt) {
          if (!prev || !prev.updatedAt) {
            return current;
          }
  ...
  setCurrentCategory(
    ...
    if (!resumeCourse) {
      ...
      if (findLatestCategory.completed) {
        ...
        const findNextCat = categories.find(
          (cat: ExtendedCategoryDTO) => !cat.completed
        );
        if (findNextCat) {
  ...
  setCurrentCourse(
    ...
    if (!resumeCourse) {
      ...
      if (findLatestCourse.completed) {
        // If it's completed, find the next category.
        const findNextCourse = currentCategory.courses.find(
    ...
    selectedCourse.results = selectedCourse.results
   ...
   */
  setCurrentExercise(
    selectedCourse: ExtendedCourseDTO,
    resumeCourse: boolean
  ): SetExerciseDTO {
    const findLastExercise = this.getLatestExercise(selectedCourse);
    const findIndex = selectedCourse.exercises.findIndex(
      (el) => el.id === findLastExercise.id
    );
    let exerciseIndex = 0;
    if (findIndex !== -1) {
      if (!resumeCourse) {
        ...
        if (!findLastExercise.completed) {
          exerciseIndex = findIndex;
          selectedCourse.exercises = selectedCourse.exercises.map(
            (el, index) => {
              const elem: ExtendedCourseExerciseDTO = { ...el };
              elem.results = el.results ? el.results : [];
              if (index >= exerciseIndex) {
  ...
  calculateSpeed(selectedCourse: ExtendedCourseDTO): number {
    if (selectedCourse.results) {
      const lastResult =
        selectedCourse.results[selectedCourse.results.length - 1];
      const speed = Math.round(
        (lastResult.characters * 60000) / lastResult.time
      );
      return speed;
    }
    return 0;
  ...
  calculateAccuracy(selectedCourse: ExtendedCourseDTO): number {
    if (selectedCourse.results) {
      ...
      const accuracy = Math.round(
        ((lastResult.characters - lastResult.mistakes) * 100) /
  ...
  calculateCourseProgress(selectedCourse: ExtendedCourseDTO): number {
    const completedExercises = selectedCourse.exercises.filter(
      (exercise: ExtendedCourseExerciseDTO) => exercise.completed
    );
